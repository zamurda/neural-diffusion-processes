...
def _make_batch(data: Mapping,
    ...
    for c in colnames:
        ...
        #     feature.append(jnp.take(jnp.array(data[c]), indices[i])) # need to take one row at a time
        # feature = jnp.stack(feature)
            ...
            for __ in range(training_config.batch_size):
                ...
                curr_batch.append(
                    jnp.sort(jax.random.choice(
                        subkey, indices, (dataset_config.sample_length,), replace=False
                            )
                        )
                    )
...
def gen_batch_eval(
    ...
    if (dir_/name).exists():
        ...
        batch_without_ctx = _make_batch(data,  dataset_config.features, dataset_config.target_index, indices=to_choose)

        num_context_points = dataset_config.sample_length // 4 if num_context_points is None else num_context_points # choose 1/4 of the sample length to be context by default
        if make_context:
            match context_type.lower():
                case 'random':
                    ...
                    for _ in range(n_draws):
                        key, subkey = jax.random.split(key)
                        to_take.append(jax.random.choice(subkey, jnp.arange(dataset_config.sample_length), shape=(num_context_points,), replace=False))
                    ...
                    for i in range(n_draws):
                        x,y = batch_without_ctx.x_target[i,:,:], batch_without_ctx.y_target[i,:,:]
                        # print(to_take[i])
                        # print(x)
                        # print(y)
                        x_c.append(x[to_take[i],:])
                        y_c.append(y[to_take[i],:])
                        mask = jnp.ones(dataset_config.sample_length, dtype=bool).at[to_take[i]].set(False).reshape(x.shape[0],1)
                        mask_x = jnp.tile(mask, (1, x.shape[-1]))
                        mask_y = jnp.tile(mask, (1, y.shape[-1]))
                        # print(x[mask_x].shape)
                        # print(y[mask_y].shape)
                        # print('\n\n')
                        x_t.append(x[mask_x].reshape(dataset_config.sample_length-num_context_points,x.shape[-1]))
                        y_t.append(y[mask_y].reshape(dataset_config.sample_length-num_context_points, y.shape[-1]))
                    x_c = jnp.stack(x_c)
                    y_c = jnp.stack(y_c)
                    x_t = jnp.stack(x_t)
                    y_t = jnp.stack(y_t)
                ...
                case 'causal': raise NotImplementedError
                case _: raise ValueError(f'context type {context_type} is not valid')
