...
ds_train = gen_dataset(
    ...
    sample_length=config.dataset.sample_length,
    kernel=_kernels[config.dataset.data],
    domain=(-1,1)
)
...
def ema_update(decay, ema_params, new_params):
    ...
    return jax.tree_map(_ema, ema_params, new_params) #changed from tree.map since jax has been downgraded
...
def process_plots(state: TrainingState) -> plt.figure:
    ...
    kern = _kernels[config.dataset.data](x)
    y = np.random.multivariate_normal([0]*100, kern).reshape(100,1)
