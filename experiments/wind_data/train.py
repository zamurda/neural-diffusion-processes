...
from util.data import gen_dataset_standard
...
EXPERIMENT = f'wind_{timestamp}_{id}_testrun'
...
ds_train, NUM_STEPS, meanvar_dict = gen_dataset_standard(
                                        DATA_DIR,
                                        config.dataset,
                                        config.training,
                                        config.seed,
...
# def plot_priors(batch: Batch, index_pos=0):
...
#         dates = [ ]
#         for t in timestamps:
#             dates.append(datetime.datetime.fromtimestamp(t).strftime('%m-%d %H:%M:%S'))
...
# def plot_conditionals(batch: Batch, index_pos=0):
...
#         )
#         mean, var = jnp.mean(samples, axis=0).squeeze(axis=1), jnp.var(samples, axis=0).squeeze(axis=1)
#         axes.plot(batch.x_target[i, :, index_pos], samples[..., 0].T, "C0", lw=1)
#         axes.plot(batch.x_target[i, :, index_pos], mean, "k")
#         axes.fill_between(
#             batch.x_target[i, :, index_pos, None].squeeze(axis=1),
#             mean - 1.96 * jnp.sqrt(var),
#             mean + 1.96 * jnp.sqrt(var),
...
#         axes.plot(xc, batch.y_context[i], "C3o")
#         # axes[i].set_xlim(-2.05, 2.05)
#         axes.scatter(batch.x_target[i, :, index_pos], batch.y_target[i].squeeze(axis=1), color='blue', marker='x')

#         timestamps = jnp.concat((batch.x_context[i, :, index_pos], batch.x_target[i, :, index_pos])).tolist()
#         dates = [ ]
#         for t in timestamps:
#             dates.append(datetime.datetime.fromtimestamp(t).strftime('%m-%d %H:%M:%S'))
#         axes.set_xticks(timestamps, labels=dates, rotation=45, fontsize=4)
#         figs.append(fig)
